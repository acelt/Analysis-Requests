...
Attributed_Pks <- TDat_LMF_Attributed %>% dplyr::select(PrimaryKey, all_of(attribute_title), Time_Period, EcologicalSiteId)
...
for(i in EcologicalSiteIds){
  md_plotlist[[i]] <- PlotMetadata_alt(dataframe = EcoSitePlots, EcologicalSite = i)
}
```
...
```{r CI nox plots static , fig.height = 4 , fig.width = 10 , echo = FALSE, include = TRUE, message=FALSE, results="asis"}
...
###
```{r CI LPI ground plot static , echo = FALSE ,  fig.height = 6 , fig.width = 10 ,  results = 'asis', message=FALSE, include=TRUE}
...
###
```{r CI Gap plot static , echo = FALSE , include = TRUE , fig.height = 7 , fig.width= 8, results="asis"}
...
Mean indicator values from 2011-2015 and 2016-2020 were compared using t-tests and Mann Whitney-U tests. 

Rows with lower p-values can help you focus on the differences which are strongest. Comparisons with a p-value of 0.2 or lower are significant at the 80% Confidence level displayed in the error bars shown in the figures above. Blank p-values indicate rows where statistical comparisons were unable to be made due to sample size limitations.
