...
output_filename <- "VirginiaMtn_winter"
...
analysis$Rating <-  factor(analysis$Rating, levels = c( "Suitable", "Unsuitable"))
...
```
```{r}
analysis_winter <- analysis
all_points_winter <-  all_points
point_weights_winter <-  point_weights
wgtcat_summary_winter <- wgtcat_summary

analysis_all <-  rbind(analysis_spring,analysis_summer,analysis_winter)
analysis_all[["Rating"]][is.na(analysis_all[["Rating"]])] <- "Suitable"

write.csv(analysis_all,
          file = paste0(output_path, "/",
                       "vmt_analysis_all",
                        date, ".csv"),
          row.names = FALSE)
