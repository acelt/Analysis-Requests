...
# removing this for cont analysis
...
# size of each strata and benchmark group 
popsize <- arc.select(arc.open(path = "C:\\Users\\alaurencetraynor\\Documents\\CO\\SLVFO Restoration\\inputs\\SLVFO Restoration.gdb\\BenchmarkGroups_Alamosa_wgtcat_intersect"))
...
for(stratum in unique(popsize$BenchmarkGroup)){
  acres <- popsize$area_acres_aea[popsize$BenchmarkGroup == stratum]
  names <- popsize$wgtcats_name[popsize$BenchmarkGroup == stratum]
...
### Index Figure
```{r}
hist(sampled_data_departure_wide$LHS1_Weighted_Mean_Index, xlab = "Standard 1 Weighted Mean Index", main = "Standard 1 Index - Alamosa-Trinchera Sub-Basin")
hist(sampled_data_departure_wide$LHS3_Weighted_Mean_Index, xlab = "Standard 3 Weighted Mean Index", main = "Standard 3 Index - Alamosa-Trinchera Sub-Basin")
```
