...
```
## Writing Outputs
```{r writing outputs, message=FALSE, warning=FALSE, eval=FALSE}
...
LHS1 <- analysis_merge[analysis_merge$Indicator %in% standard1_fields,] %>% 
  ...
  theme_bw()+
  ggtitle(label = "Standard 1 Analysis Results for San Luis")+
  theme(axis.text.x = element_text(angle = 40, hjust = 1))+
  geom_hline(aes(yintercept= 29111.2),linetype = 2, colour = "blue" )+
...
LHS3 <- analysis_merge[analysis_merge$Indicator %in% standard3_fields,] %>% 
  ...
  geom_errorbar(aes(ymin = `Lower confidence bound of acres (80%, Goodman multinomial)`,
                         ...
                         width = 0.5,position = position_dodge(width = 0.5))+
  theme_bw()+
  ggtitle(label = "Standard 3 Analysis Results for San Luis")+
  theme(axis.text.x = element_text(angle = 40, hjust = 1))+
  geom_hline(aes(yintercept= 29111.2),linetype = 2, colour = "blue" )+
  scale_y_continuous(labels = comma_format())
