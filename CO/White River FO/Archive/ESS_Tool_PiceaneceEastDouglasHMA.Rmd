...
TDat_LMF_Attributed <- AttributePlots(TDat_LMF = TDat_LMF_Time,
...
plotly::ggplotly(CFG_list_int$F048AY909CO[["Graminoid"]], tooltip = "text")%>% layout(boxmode = "group")
```

###### Static 

```{r SFG plots grass static, fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE}
plot(CFG_list_stat$F048AY909CO[["Graminoid"]])
```
...
plotly::ggplotly(CFG_list_int$F048AY909CO[["Forb"]], tooltip = "text")%>% layout(boxmode = "group")
```

###### Static 

```{r SFG plots forb static, fig.height = 5 , fig.width = 10 , echo = FALSE, include = TRUE}
plot(CFG_list_stat$F048AY909CO[["Forb"]])
```
...
plotly::ggplotly(NOX_list_int$F048AY909CO) %>% layout(boxmode = "group")
```

##### Static

```{r nox plots static , fig.height = 4 , fig.width = 10 , echo = FALSE, include = TRUE}
plot(NOX_list_stat$F048AY909CO)
```
...
plotly::ggplotly(SPECIES_list_int$F048AY909CO[["Graminoid.Annual"]] , tooltip = "text")%>% layout(boxmode = "group")
...
plot(SPECIES_list_stat$F048AY909CO[["Graminoid.Annual"]])
```
...
plotly::ggplotly(SPECIES_list_int$F048AY909CO[["Graminoid.Perennial"]] , tooltip = "text")%>% layout(boxmode = "group")
...
SPECIES_list_stat$F048AY909CO[["Graminoid.Perennial"]]
...
plotly::ggplotly(SPECIES_list_int$F048AY909CO[["Forb.Annual"]] , tooltip = "text")%>% layout(boxmode = "group")
...
plotly::ggplotly(SPECIES_list_int$F048AY909CO[["Forb.Perennial"]] , tooltip = "text")%>%layout(boxmode = "group")
...
SPECIES_list_stat$F048AY909CO[["Forb.Perennial"]]
...
plotly::ggplotly(GC_list_int$F048AY909CO)%>%layout(boxmode ="group")
...
plotly::ggplotly(GAP_list_int$F048AY909CO, tooltip = "text")%>% layout(boxmode = "group")
...
plotly::ggplotly(SS_list_int$F048AY909CO, tooltip = "text") %>% layout(boxmode = "group")
...
GeoTiff_list <- list()
# this may take a million years to run (many GBs of data)

for(i in as.numeric(c(time_period1,time_period2))){
  GeoTiff_list[[i]] <- download.file(url = paste0("http://rangeland.ntsg.umt.edu/data/rap/rap-vegetation-cover/v2/vegetation-cover-v2-",i,".tif"),
                                     destfile = paste0(WorkingFolder, "/RapImage", i, ".tif",
                                                       method = "auto")
  )
}
```
