...
#check the required packages packages and install uninstalled ones if necessary
project_packages <- c("raster", "dplyr", "sf", "rgeos", "randomForest", "tmap", "caret")
packages_to_download <- project_packages[!(project_packages %in% installed.packages()[,"Package"])]
if(length(packages_to_download)) install.packages(packages_to_download)
...
setwd("C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\CO\\Uncomphagre FO")    

### Reading in and manipulating data ----

#North Delta data ----

#read in csv of north delta data
ndelta<- read.csv("input_data/ND_LHA_2018.csv")
...
# and reproject ndelta shp
ndelta_shp<-spTransform(ndelta_shp, ndelta_allotments@proj4string)

# make new field defining not meeting and meeting as 0/1
...
#make slightly larger extent for plotting later
...
#read in landfire data
evt_class5 <- raster("landfire/evt_class_focal5.tif")
    ### Note: this is landfire existing vegetation type class. Already manipulated with a focal 5x5 analysis to reduce noise 

# read in, reproject, and rasterize outputs from soil data viewer
...
# Make a layer of allotment number
...
sample_strata<- shapefile("//blm/dfs/loc/EGIS/CO/GIS/giswork/ufo/projects/vegetation/Land Health Assessments/North Delta/layers/ND_LHA_2017/UFO_north_delta_intensification_2018_sample_frame.shp")

# need to make the text fields into a number so that strata can be rasterized
    # these numbers are not meaningful, but there may be a slight impact from which strata is assigned which number
        # i think random forest is a lot better at sorting through that than a linear regression, eg
sample_strata@data$Strata_com_num<-NA
sample_strata@data$Strata_com_num[sample_strata@data$Strata_com=="Salt Flats"]<-1
sample_strata@data$Strata_com_num[sample_strata@data$Strata_com=="Clayey Salt Desert"]<-2
sample_strata@data$Strata_com_num[sample_strata@data$Strata_com=="Stony Saltdesert"]<-3
sample_strata@data$Strata_com_num[sample_strata@data$Strata_com=="Loamy Foothills"]<-4
...
#change deer basin to different allotment number
    # technically deer basin and midway are one allotment, but since they are separated from each other, very different, 
    # and we're considering management changes between them, we separated them in the model here
db_extent<-extent(allotment_num)
db_extent[3]<-4300100

allotment_num[db_extent][allotment_num[db_extent] == 14019] <- 14020

## Extract the raster data onto the North Delta shapefile ----
...
ndelta_shp$evt_class5<-extract(evt_class5, ndelta_shp, method="simple")

#change meeting / not meeting to factor for modeling (rather than a number)
...
d<-ndelta_shp@data[,c(1:10,140:ncol(ndelta_shp@data))]

# These are other layers not currently using in our model, but can experiment with them  ----
    # They may make results slightly more accurate, but we valued simplicity and having results
    # That made sense on the ground. These look more pixely and don't follow features
    # Like soil map units or allotments as intuitively
    # Running the models with these layers is done lower down, after running the more simple model

tree_cov<-crop(projectRaster(raster("landfire/tree_cover_class.tif"),slope,method="ngb"),slope)
ndelta_shp$tree_cov<-extract(tree_cov, ndelta_shp, method="simple")

veg_departure <-crop(projectRaster(raster("landfire/veg_departure.tif"),slope,method="ngb"),slope)
ndelta_shp$veg_departure<-extract(veg_departure, ndelta_shp, method="simple")

veg_height <-crop(projectRaster(raster("landfire/veg_height.tif"),slope,method="ngb"),slope)
ndelta_shp$veg_height<-extract(veg_height, ndelta_shp, method="simple")
...
# Run Random Forest on standard 1 ----
rf_std1_factor<- randomForest(Std_1_num_factor ~
                         sample_strata+allotment_num+clay+range_prod+evt_class5+elevation,
                       data= ndelta_shp@data,
                       ntree=5000, mtry=2, importance=TRUE,corr.bias=F)

rf_std1_factor   # this spits out some info about the random forest
    # the confusion matrix here is the OOB confusion matrix

# a more normal confusion matrix showing the random forest's prediction at each input point
  # compared to the true values can be made with this:
ndelta_pred<-ndelta_shp@data
ndelta_pred$rf_predicted_std1_factor<-predict(rf_std1_factor, ndelta_pred)
#
confusionMatrix(data=ndelta_pred$rf_predicted_std1_factor,reference=ndelta_pred$Std_1_num_factor)

#or you can compare prediction to reference point by point
ndelta_pred$rf_predicted_std1_factor
ndelta_pred$Std_1_num_factor

#  variable importance plot
varImpPlot(rf_std1_factor)



# Run Random Forest on standard 3 ----
rf_std3_factor<- randomForest(Std_3_num_factor ~
                                    sample_strata+allotment_num+clay+range_prod+evt_class5+elevation,
                                  data= ndelta_shp@data,
                                  ntree=5000, mtry=2, importance=TRUE,corr.bias=F)
...
rf_std3_factor   # this spits out some info about the random forest
# the confusion matrix here is the OOB confusion matrix

# a more normal confusion matrix showing the random forest's prediction at each input point can be made with this:
ndelta_pred<-ndelta_shp@data
ndelta_pred$rf_predicted_std3_factor<-predict(rf_std3_factor, ndelta_pred)
#
confusionMatrix(data=ndelta_pred$rf_predicted_std3_factor,reference=ndelta_pred$Std_3_num_factor)

#or you can compare prediction to reference point by point
ndelta_pred$rf_predicted_std3_factor
ndelta_pred$Std_3_num_factor

#  variable importance plot
varImpPlot(rf_std3_factor)


#

# save out rf models----
#saveRDS(rf_std1_factor,"rf_models/std1_july21.rds")
#saveRDS(rf_std3_factor,"rf_models/std3_july21.rds")


# load past rf models----
#rf_std1_factor<-readRDS("rf_models/std1_july21.rds") 
#rf_std3_factor<-readRDS("rf_models/std3_july21.rds") 


# Make raster stack for prediction -----
pred_stack<-stack(sample_strata_ras,allotment_num,clay,range_prod,evt_class5,elevation)
names(pred_stack)<- c("sample_strata","allotment_num","clay","range_prod","evt_class5","elevation")


## Make standard 1 raster ----
std1_pred_raster<- raster::predict(object=pred_stack,model=rf_std1_factor,na.rm=T,inf.rm=T,format="GTiff",progress="text",datatype="INT1U")

#set as numeric rather than factor
std1_pred_raster<- setValues(raster(std1_pred_raster), std1_pred_raster[])

#then mask to blm land
std1_pred_raster<- mask(std1_pred_raster, blm)


# how much is meeting, how much is not meeting?
r_std1_petrie<-mask(std1_pred_raster, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Petrie Mesa",])
r_std1_point<-mask(std1_pred_raster, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Point Creek",])
r_std1_dbm<-mask(std1_pred_raster, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Deer Basin - Midway",])
...
# by acres
ncell(r_std1_petrie[r_std1_petrie==1]) * res(std1_pred_raster)[1]^2 / 4046.86
ncell(r_std1_petrie[r_std1_petrie==0]) * res(std1_pred_raster)[1]^2 / 4046.86

ncell(r_std1_point[r_std1_point==1]) * res(std1_pred_raster)[1]^2 / 4046.86
ncell(r_std1_point[r_std1_point==0]) * res(std1_pred_raster)[1]^2 / 4046.86

ncell(r_std1_dbm[r_std1_dbm==1]) * res(std1_pred_raster)[1]^2 / 4046.86
ncell(r_std1_dbm[r_std1_dbm==0]) * res(std1_pred_raster)[1]^2 / 4046.86


#make palette for mapping
pal<-c("#b52309","#048f53")

#make map
tm_shape(std1_pred_raster, bbox=new_ext) + 
  ...
  #tm_credits(text="Draft still in refinement", fontface="italic", size=1.3, position = c(0.4,0.95))+
...
# save out the raster
#writeRaster(std1_pred_raster, "output_rasters/std1_rf_july21",format="GTiff")



## Make standard 3 rasters----
std3_pred_raster<-raster::predict(object=pred_stack,model=rf_std3_factor,na.rm=T,inf.rm=T,format="GTiff",progress="text",datatype="INT1U")

#set as numeric rather than factor
std3_pred_raster <- setValues(raster(std3_pred_raster), std3_pred_raster[])

#then mask to blm
std3_pred_raster<-mask(std3_pred_raster, blm)


# how much is meeting, how much is not meeting
r_std3_petrie<-mask(std3_pred_raster, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Petrie Mesa",])
r_std3_point<-mask(std3_pred_raster, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Point Creek",])
r_std3_dbm<-mask(std3_pred_raster, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Deer Basin - Midway",])
...
#acres
ncell(r_std3_petrie[r_std3_petrie==1]) * res(std3_pred_raster)[1]^2 / 4046.86
ncell(r_std3_petrie[r_std3_petrie==0]) * res(std3_pred_raster)[1]^2 / 4046.86

ncell(r_std3_point[r_std3_point==1]) * res(std3_pred_raster)[1]^2 / 4046.86
ncell(r_std3_point[r_std3_point==0]) * res(std3_pred_raster)[1]^2 / 4046.86

ncell(r_std3_dbm[r_std3_dbm==1]) * res(std3_pred_raster)[1]^2 / 4046.86
ncell(r_std3_dbm[r_std3_dbm==0]) * res(std3_pred_raster)[1]^2 / 4046.86

#set palette
pal<-c("#b52309","#048f53")

#make map
tm_shape(std3_pred_raster,bbox=new_ext) + 
  tm_raster(n=2,palette = pal,
            title="Standard 3", 
            breaks = c(0,0.5,1),
            labels = c("Not Meeting", "Meeting"))+ 
...
# save out the raster
#writeRaster(std3_pred_raster, "output_rasters/std3_rf_july21",format="GTiff")



#### Make rasters for the more complicated models -----

rf_std1_factor_all<- randomForest(Std_1_num_factor ~
                                    sample_strata+allotment_num+clay+range_prod+evt_class5+elevation+
                                    tree_cov+veg_departure+veg_height+awc+org_matter,
                                  data= ndelta_shp@data,
                                  ntree=5000, mtry=3, importance=TRUE,corr.bias=F)

rf_std3_factor_all<- randomForest(Std_3_num_factor ~
                                    sample_strata+allotment_num+clay+range_prod+evt_class5+elevation+
                                    tree_cov+veg_departure+veg_height+awc+org_matter,
                                  data= ndelta_shp@data,
                                  ntree=5000, mtry=2, importance=TRUE,corr.bias=F)
...
pred_stack_all<-stack(sample_strata_ras,allotment_num,clay,range_prod,evt_class5,elevation,
                      tree_cov,veg_departure,veg_height,awc,org_matter)
names(pred_stack_all)<- c("sample_strata","allotment_num","clay","range_prod","evt_class5","elevation",
                          "tree_cov","veg_departure","veg_height","awc","org_matter")

# Std 1 rasters
std1_pred_raster_all<- raster::predict(object=pred_stack_all,model=rf_std1_factor_all,na.rm=T,inf.rm=T,format="GTiff",progress="text",datatype="INT1U")

#set as numeric rather than factor
std1_pred_raster_all<- setValues(raster(std1_pred_raster_all), std1_pred_raster_all[])

#then mask to blm land
std1_pred_raster_all<- mask(std1_pred_raster_all, blm)


# Std 3 rasters
std3_pred_raster_all<- raster::predict(object=pred_stack_all,model=rf_std3_factor_all,na.rm=T,inf.rm=T,format="GTiff",progress="text",datatype="INT1U")

#set as numeric rather than factor
std3_pred_raster_all<- setValues(raster(std3_pred_raster_all), std3_pred_raster_all[])

#then mask to blm land
std3_pred_raster_all<- mask(std3_pred_raster_all, blm)
...
tm_shape(std1_pred_raster_all,bbox=new_ext) + 
  tm_raster(n=2,palette = pal,
            title="Standard 1", 
            breaks = c(0,0.5,1),
            labels = c("Not Meeting", "Meeting"))+ 
...
tm_shape(std3_pred_raster_all,bbox=new_ext) + 
  tm_raster(n=2,palette = pal,
            title="Standard 3", 
            breaks = c(0,0.5,1),
            labels = c("Not Meeting", "Meeting"))+ 
...
# This doesn't work that well, but it gives an idea of how these trees work

##  Skip all the way to the bottom to see examples
...
# these give you a sense of how the random forest trees split and how a meeting or not meeting determination is made
# these are just example trees though

plot.getTree(rf_std1_factor, k=4700, depth=5)

plot.getTree(rf_std3_factor, k=4670, depth=5)
