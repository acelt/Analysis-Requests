...
# need to make the text fields into a number to rasterize
...
#change meeting / not meeting to factor for modeling -----
...
# other layers not currently using ----
tree_cov <- projectRaster(raster("../landfire/tree_cover_class.tif"),crs=crs(ndelta_shp@proj4string))
...
#ndelta_shp$bps<-extract(bps, ndelta_shp, method="simple")
...
#evt_order<-raster("../landfire/evt_order.tif")
#evt_order<-crop(projectRaster(evt_order,slope,method="ngb"),slope)
#evt_order3 <- focal(evt_order, w=matrix(1,nrow=3,ncol=3), fun=modal)
#evt_order5 <- focal(evt_order, w=matrix(1,nrow=5,ncol=5), fun=modal)

#evt_class<-raster("../landfire/evt_class.tif")
#evt_class<-crop(projectRaster(evt_class,slope,method="ngb"),slope)
#evt_class3 <- focal(evt_class, w=matrix(1,nrow=3,ncol=3), fun=modal)
#evt_class5 <- focal(evt_class, w=matrix(1,nrow=5,ncol=5), fun=modal)


# Standard 1 RF----
rf_std1<- randomForest(Std_1_num ~
                        sample_strata+allotment_num+clay+range_prod+evt_class5+elevation+slope,
...
# Standard 1 RF----
rf_std1_fact<- randomForest(Std_1_num_factor ~
                         sample_strata+allotment_num+clay+range_prod+evt_class5+elevation+slope,
                       data= ndelta_shp@data,
                       ntree=5000, mtry=3, importance=TRUE,corr.bias=F)
...
rf_std1_fact
varImpPlot(rf_std1_fact)


test<-ndelta_shp@data
test$rf_predicted_std1<-predict(rf_std1, test)
test$rf_predicted_std1
test$rf_predicted_std1[test$rf_predicted_std1<0.5]<-0
test$rf_predicted_std1[test$rf_predicted_std1>0.495]<-1
test$rf_predicted_std1
test$Std_1_num_factor

test$rf_predicted_std1_fact<-predict(rf_std1_fact, test)
test$rf_predicted_std1_fact
test$Std_1_num_factor

# save out rf model
#saveRDS(rf_std1,"../rf_models/std1_march5_landfire_evtclass.rds")
...
# Standard 3  RF----
rf_std3<- randomForest(Std_3_num ~
...
# Standard 3 RF---
rf_std3_fact<- randomForest(Std_3_num_factor ~
                              sample_strata+allotment_num+clay+range_prod+evt_class5+elevation,
                            data= ndelta_shp@data,
                            ntree=5000, mtry=3, importance=TRUE,corr.bias=F)
...
rf_std3_fact
varImpPlot(rf_std3_fact)


test$rf_predicted_std3<-predict(rf_std3, test)
test$rf_predicted_std3
test$rf_predicted_std3[test$rf_predicted_std3<0.5]<-0
test$rf_predicted_std3[test$rf_predicted_std3>0.495]<-1
test$rf_predicted_std3
test$Std_3_num_factor

test$rf_predicted_std3_fact<-predict(rf_std3_fact, test)
test$rf_predicted_std3_fact
test$Std_3_num_factor



# save out rf model
#saveRDS(rf_std3,"../rf_models/std3_march5_landfire_evtclass.rds")
...
#CONFUSON----
std1_num_confusion<-confusionMatrix(data=as.factor(test$rf_predicted_std1),
                                    reference=as.factor(test$Std_1_num_factor))

std1_fact_confusion<-confusionMatrix(data=as.factor(test$rf_predicted_std1_fact),
                                     reference=as.factor(test$Std_1_num_factor))
std1_num_confusion
std1_fact_confusion
...
std3_num_confusion<-confusionMatrix(data=as.factor(test$rf_predicted_std3),
                                    reference=as.factor(test$Std_3_num_factor))

std3_fact_confusion<-confusionMatrix(data=as.factor(test$rf_predicted_std3_fact),
                                     reference=as.factor(test$Std_3_num_factor))
std3_num_confusion
std3_fact_confusion

mean(test$rf_predicted_std3-as.numeric((as.character(test$Std_3_num_factor)))^2)

mean(as.numeric(test$rf_predicted_std3_fact)-(as.numeric(test$Std_3_num_factor))^2)


# Make raster stack for prediction -----
pred_stack<-stack(sample_strata_ras,allotment_num,clay,range_prod,evt_class5,elevation,slope)
names(pred_stack)<- c("sample_strata","allotment_num","clay","range_prod","evt_class5","elevation","slope")


## Standard 1 maps ----
std1_pred<-raster::predict(object=pred_stack,model=rf_std1,na.rm=T,inf.rm=T,format="GTiff",progress="text",datatype="INT1U")
std1_pred[std1_pred<0.5]<-0
std1_pred[std1_pred>0.495]<-1
std1_pred_fact<-raster::predict(object=pred_stack,model=rf_std1_fact,na.rm=T,inf.rm=T,format="GTiff",progress="text",datatype="INT1U")

#set as numeric rather than factor
std1_pred_fact <- setValues(raster(std1_pred_fact), std1_pred_fact[])

#then mask to blm land
std1_pred<-mask(std1_pred, blm)
std1_pred_fact<-mask(std1_pred_fact, blm)




# how much is meeting, how much is not meeting
r_std1_petrie<-mask(std1_pred_fact, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Petrie Mesa",])
r_std1_point<-mask(std1_pred_fact, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Point Creek",])
r_std1_dbm<-mask(std1_pred_fact, ndelta_allotments[ndelta_allotments$ALLOT_NAME=="Deer Basin - Midway",])
...
jpeg("../plots/ndelta_2018_std1_randomforest_march5_landfire_evt_class.jpeg", width=8,height=8,units="in",res=1000)
...
# write raster
writeRaster(std1_pred, "../output_rasters/std1_rf_march5_landfire_evt_class2",format="GTiff")



## Standard 3 maps----
std3_pred<-raster::predict(object=pred_stack,model=rf_std3,na.rm=T,inf.rm=T,format="GTiff",progress="text",datatype="INT1U")
std3_pred[std3_pred<0.5]<-0
std3_pred[std3_pred>0.495]<-1
std3_pred_fact<-raster::predict(object=pred_stack,model=rf_std3_fact,na.rm=T,inf.rm=T,format="GTiff",progress="text",datatype="INT1U")

#set as numeric rather than factor
std3_pred_fact <- setValues(raster(std3_pred_fact), std3_pred_fact[])

#then mask to blm
std3_pred<-mask(std3_pred, blm)
std3_pred_fact<-mask(std3_pred_fact, blm)
...
jpeg("../plots/ndelta_2018_std3_randomforest_march5_landfire_evtclass.jpeg", width=8,height=8,units="in",res=1000)
...
# write raster
writeRaster(std3_pred, "../output_rasters/std3_rf_march5_landfire_evtclass2",format="GTiff")
...
plot.getTree(rf_std3, k=5000, depth=7)
