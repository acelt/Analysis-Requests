...
dir <- "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\CO\\Uncomphagre FO"

# load past rf models
rf_std1<-readRDS(paste0(dir,"/rf_models/std1_march5_landfire_evtclass.rds"))
rf_std3<-readRDS(paste0(dir,"/rf_models/std3_march5_landfire_evtclass.rds"))

#look at std1
rf_std1
randomForest::varImpPlot(rf_std1)
...
#make nice variable importance plots ----

# STD 1 ----
rownames(rf_std1$importance)<- c("Sample Strata","Allotment Number","Clay Percent","Range Productivity",
                                 "Existing Vegetation Type Class","Elevation")

std1_imp <- varImpPlot(rf_std1) # let's save the varImp object

# this part just creates the data.frame for the plot part
std1_imp <- as.data.frame(std1_imp)
std1_imp$varnames <- rownames(std1_imp) # row names to column
rownames(std1_imp) <- NULL  
colnames(std1_imp)<-c("PercentIncreaseMSE","IncNodePurity","varname")


std1<-ggplot(std1_imp, aes(x=reorder(varname, PercentIncreaseMSE), y=PercentIncreaseMSE)) + 
  ...
  theme(panel.grid.major.y = element_line(colour = "grey80",linetype="dotted",size=0.5))+
...
# STD 3 ----
rownames(rf_std3$importance)<- c("Sample Strata","Allotment Number","Clay Percent","Range Productivity",
                                 "Existing Vegetation Type Class","Elevation")

std3_imp <- varImpPlot(rf_std3) # let's save the varImp object

# this part just creates the data.frame for the plot part
std3_imp <- as.data.frame(std3_imp)
std3_imp$varnames <- rownames(std3_imp) # row names to column
rownames(std3_imp) <- NULL  
colnames(std3_imp)<-c("PercentIncreaseMSE","IncNodePurity","varname")



std3<-ggplot(std3_imp, aes(x=reorder(varname, PercentIncreaseMSE), y=PercentIncreaseMSE)) + 
  geom_point(shape=21,size=2,fill="white",color="black") +
  ylab("Mean Accuracy Decrease (%)") +
  xlab("") +
  ggtitle("Standard 3")+
...
jpeg("../plots/variable_importance_std1_std3.jpeg",width=11,height = 4.5,units = "in",res=1000)
grid.arrange(std1,std3,ncol=2,nrow=1)
dev.off()
...
jpeg("../plots/rf_std3_variable_importance.jpeg",width=5.5,height = 4.5,units = "in",res=1000)
