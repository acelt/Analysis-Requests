...
ndelta_allotments<-shapefile("../north_delta_allotments/north_delta_allotments.shp")
...
# read in blm land ----
blm<-shapefile("../blm_land/blm_dissolve_ufo.shp")
...
writeRaster(evt_order3,"../landfire/evt_order_focal3",format="GTiff")
writeRaster(evt_order5,"../landfire/evt_order_focal5",format="GTiff")
writeRaster(evt_class3,"../landfire/evt_class_focal3",format="GTiff")
writeRaster(evt_class5,"../landfire/evt_class_focal5",format="GTiff")
...
ndelta_shp$evt_order<-extract(evt_order, ndelta_shp, method="simple")
ndelta_shp$evt_order3<-extract(evt_order3, ndelta_shp, method="simple")
ndelta_shp$evt_order5<-extract(evt_order5, ndelta_shp, method="simple")
ndelta_shp$evt_class<-extract(evt_class, ndelta_shp, method="simple")
ndelta_shp$evt_class3<-extract(evt_class3, ndelta_shp, method="simple")
...
rf_std1<- randomForest(Std_1_num_factor ~
                         allotment_num+sample_strata+clay+range_prod+evt_class5+elevation,
...
rf_std3<- randomForest(Std_3_num_factor ~
                         allotment_num+sample_strata+clay+range_prod+evt_class5+elevation,
                       data= ndelta_shp@data,
                       ntree=5000, mtry=2, importance=TRUE,corr.bias=T)
...
pred_stack<-stack(allotment_num, sample_strata_ras,clay,range_prod,evt_class5,elevation)
names(pred_stack)<- c("allotment_num","sample_strata","clay","range_prod","evt_class5","elevation")
...
# write raster
writeRaster(std1_pred, "../output_rasters/std1_rf_march5_landfire_evt_class",format="GTiff")
...
# write raster
writeRaster(std3_pred, "../output_rasters/std3_rf_march5_landfire_evtclass",format="GTiff")
