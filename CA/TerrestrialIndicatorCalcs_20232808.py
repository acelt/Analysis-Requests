...
def RunCalcs(adminstate,item_id):
    ...
    item = gis.content.get(item_id)
    plotlyr = item.layers[0]
    ploturl = plotlyr.url
    plotLayer = arcpy.TableToTable_conversion(ploturl, "memory", "plotLayer")
    ...
    with arcpy.da.SearchCursor(speciesLayer, fields, whereClause) as cursor:
        for row in cursor:
    ...
    nonnoxList = list(set(nonnoxList))
...
# fcs_dict = {"NV":'0355e7481258440c9ef6cd02ea52e004'}
for key in fcs_dict:
    adminstate = key
...
itemid = 'f325378b9f4c477197fac83ae27b8e28'
