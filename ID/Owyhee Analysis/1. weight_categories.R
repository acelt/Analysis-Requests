...
sdd_clip <- function(reportingunit,
  ...
  if (is.null(lmf_strata)) {
    ...
    if (is.null(lmf_path)) {
      ...
      # To prevent issues related to multisurface polygons, we can convert to multipolygon
      # ensure_multipolygons <- function(X) {
      #   tmp1 <- tempfile(fileext = ".gpkg", tmpdir = "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\temp")
      #   tmp2 <- tempfile(fileext = ".gpkg", tmpdir = "C:\\Users\\alaurencetraynor\\Documents\\2021\\Analysis\\temp")
      #   sf::st_write(X, tmp1)
      #   gdalUtils::ogr2ogr(tmp1, tmp2, f = "GPKG", nlt = "MULTIPOLYGON")
      #   Y <- sf::st_read(tmp2)
      ...
      #sf::st_geometry_type(lmf_strata_sf)
        
      #lmf_strata_sf <- ensure_multipolygons(lmf_strata_sf)
    }
  } else {
  ...
  # Plot a visual confirmation that everything is going according to plan
   # ggplot2::ggplot() +
   #   ggplot2::geom_sf(data = reporting_unit_sf,
   #           color = "Red") +
   #   ggplot2::geom_sf(data = pointvisits_relevant_sf)+
   #   ggplot2::geom_sf(data = lmf_strata_sf,
   #                    color = "Green",
   #                    ggplot2::aes(fill = stratum))+
   #   ggplot2::geom_sf(data = strata_buffered_sf,
   #                    fill = "Black")
  ...
  if (is.null(lmf_strata)) {
    ...
    # Troubleshooting WKB ParseException error
    #polygons_possibly_fixed <- sf::st_cast(lmf_strata_sf, to = "GEOMETRYCOLLECTION")
    
    #lmf_fixed <- sf::st_collection_extract(polygons_possibly_fixed, "POLYGON")
        
    # Possibly followed by this step
    #lmf_sf <- sf::st_as_sf(polygons_possibly_fixed)
    ...
    # check these actually overlap
    # ggplot2::ggplot() +
    #     ggplot2::geom_sf(data = reporting_unit_sf,
    #             color = "Red") +
    #     ggplot2::geom_sf(data = lmf_strata_sf, 
    #                      col = "Green") +
    #   ggplot2::geom_sf(data = lmf_strata_buffered_sf,
    #                    fill = "Black")
    ...
    lmf_strata_clipped_sf[["name"]] <- paste0("LMF_", lmf_strata_clipped_sf[["stratum"]])
    ...
    lmf_strata_for_combining_sf <- lmf_strata_clipped_sf[, c("unique_id","name")]
  }
  ...
  if (nrow(strata_sf) > 0) {
    ...
    strata_clipped_sf[["name"]] <- strata_clipped_sf[["TerrestrialDesignStratumID"]]
    ...
    strata_for_combining_sf <- strata_clipped_sf[, c("unique_id","name")]
  } else {
  ...
  if (nrow(sampleframe_sf) > 0) {
    ...
    sampleframe_clipped_sf[["name"]] <- sampleframe_clipped_sf[["TerrestrialSampleFrameID"]]
    ...
    sampleframe_for_combining_sf <- sampleframe_clipped_sf[, c("unique_id","name")]
  } else {
...
wgtcat_gen <- function(reportingunit,
  ...
  # Buffer by 0 to remove geometry errors
  reporting_unit_sf <- sf::st_buffer(reporting_unit_sf,
                                     dist = 0)
  ...
  if (is.null(lmf_strata)) {
    ...
    # Buffer by 0 to correct geometry errors
    lmf_strata_sf <- sf::st_simplify(lmf_strata_sf)
