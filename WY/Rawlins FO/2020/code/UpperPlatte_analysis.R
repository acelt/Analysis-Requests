...
sampled_points@data[["Reporting.Unit"]] <- "Upper Platte"
...
design_points <- sf::st_read(dsn = paste(path_spatial, "Platte Analysis.gdb", sep = "/"),
                             layer = "design_points_upper_join",
                             stringsAsFactors = FALSE)
## Standardize projection
...
all_points[["Reporting.Unit"]] <- "Upper Platte"
weight_info[["point.weights"]][["Reporting.Unit"]] <- "Upper Platte"

analysis <- analyze(benchmarked_points = benchmarked_points,
                    point_weights = weight_info[["point.weights"]],
                    id_var = "unique_id",
...
names(all_points@data) <- c("plot_id", "fate", "wgtcat","x","y", "reporting_unit",
                            "wgt", "Bare Soil Cover","Invasive Annual Grass Cover","Sagebrush Height", "Sagebrush Cover", "Invasive Annual Forb Cover","Non Invasive Perennial Grass Cover", "Non Invasive Perennial Forb Cover","Non Invasive Perennial Grass Height")
...
# need to document which benchamrk group each plot falls in?
#################################################################
...
analysis2 <-subset(analysis, Category!="Meeting"&Category !="Not Meeting") # reorder factors

analysis2$Category <- factor(analysis2$Category,levels(factor(analysis2$Category))[c(3,4,1,2)])

ggplot2::ggplot(data = analysis2,
...
ggsave(device = "jpeg", dpi = 400, width = 17, filename = "upper_platte1.jpeg", path = output_path)
...
ggplot2::ggplot(data = analysis1,
  ...
  scale_fill_manual(values = c("#FF0000",
                               "#3CE922"))
ggsave(device = "jpeg", dpi = 400, width = 12, filename = "upper_platte2.jpeg", path = output_path)
